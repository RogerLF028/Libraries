...
# Lista de todas as tabelas que serão geradas (na ordem desejada para os offsets)
TABLES_ORDER = ['Transistor_BJT', 'Transistor_FET', 'Transistor_IGBT', 'Transistor_General']

# Define um offset de 100000 para cada tabela (0, 100000, 200000, 300000)
offsets = {table: i * 100000 for i, table in enumerate(TABLES_ORDER)}
# Contador sequencial interno por tabela (começa em 0)
counters = {table: 0 for table in TABLES_ORDER}

# Dicionário para armazenar as linhas de valores de cada tabela
table_values = {table: [] for table in TABLES_ORDER}
...
# Função para determinar o tipo do transistor (Info1)
...
with open(csv_path, 'r', encoding='utf-8') as csvfile:
    ...
    for row in reader:
        ...
        # Incrementa o contador interno da tabela e calcula o número global
        counters[target_table] += 1
        mypn_number = offsets[target_table] + counters[target_table]
        mypn = f"EL-TRA-{mypn_number:06d}"
        ...
        # Função auxiliar para formatar valores SQL
        def sql_value(val):
            if val is None:
                return 'NULL'
