...
# Valores abaixo de 1Ω (comuns em 0402)
...
# Datasheet URL (pode ser a mesma para todos)
DATASHEET_URL = "https://www.yageo.com/upload/media/product/products/datasheet/RC0402.pdf"
...
def generate_yageo_pn(res_ohms, tol):
    """Gera part number estilo Yageo: RC0402FR-07xxxxL (simplificado)"""
    tol_code = "FR"  # 1%
    val_str = format_resistance(res_ohms)
    return f"RC0402{tol_code}-07{val_str}L"
...
def main():
    ...
    output_file = os.path.join(script_dir, "insert_resistor_0402.sql")

    resistencias = generate_unique_resistance_values()
    print(f"Total de valores únicos: {len(resistencias)}")

    # Lista de colunas relevantes, agora incluindo Datasheet, Created_At, Created_By
    columns = [
    ...
    for res_ohms in resistencias:
        ...
        name = f"RES_0402_{res_str}_{TOLERANCE}"
        description = f"Resistor SMD 0402 {res_str} {TOLERANCE} {POWER_RATING}"
        info1 = f"{TOLERANCE}"
        info2 = f"{POWER_RATING}"
        mypn = f"EL-RES-{mypn_counter:06d}"
        mypn_counter += 1

        mfg_pn = generate_yageo_pn(res_ohms, TOLERANCE)

        # Lista de valores na mesma ordem das colunas
        # Para Created_At usamos a função SQL datetime('now') que será avaliada no momento da inserção
        # Portanto, colocamos a string "datetime('now')" sem aspas no SQL final.
        values = [
            mypn,                           # MyPN
            name,                           # Name
        ...
        # Formatar valores: strings com aspas simples escapadas, exceto para a função SQL
        formatted_values = []
        for v in values:
            if v is None:
            ...
            elif v == "datetime('now')":  # Função SQL, não colocar aspas
    ...
    # Gerar INSERT único
    header = f"INSERT INTO Resistor_0402 ({', '.join(columns)}) VALUES\n"
    body = ",\n".join(values_lines) + ";"

    with open(output_file, "w", encoding="utf-8") as f:
        f.write("-- Script de inserção para Resistor_0402\n")
        f.write("-- Baseado nas séries E24 e E96 + valores <1Ω, tolerância 1%, potência 1/16W\n")
        f.write("-- Fabricante: Yageo (part numbers simulados)\n\n")
        f.write(header)
        f.write(body)
        f.write("\n")

    print(f"Arquivo gerado com {len(values_lines)} inserts: {output_file}")
