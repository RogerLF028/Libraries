...
# -*- coding: utf-8 -*-
"""
Gerador de comandos SQLite para conectores Phoenix MSTB
Arquivo: Gerar_Insert/gerar_insert_mstb.py
"""
...
class PhoenixMSTBGenerator:
    def __init__(self, symbol_file, footprint_file, output_file):
        ...
        self.counter = 520000  # Começar depois dos GMSTB (510000)
    ...
    def filter_phoenix_footprints(self):
        print("\n🔍 Filtrando footprints Phoenix MSTB...")
        keywords = [
            'PhoenixContact_MSTBA_',
            'PhoenixContact_MSTBVA_',
            'PhoenixContact_MSTBV_',
            'PhoenixContact_MSTB_'
        ]
        ...
        print(f"   ✅ {len(self.phoenix_footprints)} footprints Phoenix MSTB encontrados")
        return True
    
    def parse_footprint(self, name):
        """Parse footprints da família MSTB"""
        data = {
            'footprint': name,
            'has_flange': 0,
    ...
    def get_current_rating(self, positions):
        """
        Corrente nominal para MSTB 2,5mm²
        Valores típicos da série (levemente maiores que GMSTB)
        """
        if positions <= 4:
            return 16.0
        elif positions <= 8:
    ...
    def get_voltage_rating(self, voltage_series):
        ...
        """
        return 400 if voltage_series == 'high_voltage' else 320  # MSTB usa 320V como padrão
    ...
    def generate_name(self, data):
        """Gera nome no formato CON_BLOCK_PHOENIX_MSTB_..."""
        series = data['series']
        pos = data['positions']
        pitch = data['pitch']
    ...
    def run(self):
        print("=" * 60)
        print("🔧 GERADOR DE INSERTS PARA CONECTORES PHOENIX MSTB")
        print("=" * 60)
...
def main():
    ...
    generator = PhoenixMSTBGenerator(
        ...
        output_file=str(base_dir / "sqlite_commands_mstb.txt")
    )
    generator.run()
